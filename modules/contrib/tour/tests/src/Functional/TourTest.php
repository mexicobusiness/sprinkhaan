...
class TourTest extends TourTestBase {
   ...
   */
  protected array $permissions = ['access tour', 'administer languages'];
  ...
  public function testTourFunctionality(): void {
    // Navigate to tour-test-1 and verify the tour_test_1 tip is found with
    // appropriate classes.
    ...
    $elements = [];
    foreach ($tips as $tip) {
      if ($tip['id'] == 'tour-test-1' && $tip['type'] == 'text' && str_contains($tip['body'], $href) && str_contains($tip['body'], 'Drupal')) {
        $elements[] = $tip;
      }
    }
    ...
    $this->assertEquals(['tour_test'], $dependencies['module']);
   ...
   */
  protected function getTourTips(): array {
    $tips = [];
   ...
   */
  protected function findTip(array $params): array {
    $tips = $this->getTourTips();
    $elements = [];
    foreach ($tips as $tip) {
